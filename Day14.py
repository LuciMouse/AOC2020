...
def diagram_rock_path(rock_path_ls, sand_source_coord,has_floor):
    """
    ...
    if has_floor:
        #add "floor" across the bottom of the cave
        floor_y_coord = num_rows + 1
        num_rows +=2

        rock_path_ls.append(
            [[zero_index_value,floor_y_coord],[num_cols-1+zero_index_value,floor_y_coord]]
        )
...
def drop_sand_floor(sand_diagram_ls, sand_coord, zero_index_value):
    """
...
def add_sand(sand_diagram_ls, sand_source_coord, zero_index_value, has_floor):
    """
    ...
    if has_floor == False:
        ...
        while not full:
            try:
                rest_posn, full = drop_sand_floor(sand_diagram_ls, sand_source_coord, zero_index_value)
            except IndexError:
                print(sand_count)
            if not full:
                sand_diagram_ls[rest_posn[1]][rest_posn[0] - zero_index_value] = 'o'
...
def sand_counter(raw_data, has_floor):
    ...
    sand_diagram_ls, zero_index_value = diagram_rock_path(rock_path_ls, sand_source_coord, has_floor)

    sand_count = add_sand(sand_diagram_ls, sand_source_coord, zero_index_value, has_floor)
    return sand_count
